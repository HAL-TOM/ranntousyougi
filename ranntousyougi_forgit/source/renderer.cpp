...
ID3D11VertexShader*     CRenderer::m_DefaultVertexShader = NULL;
ID3D11PixelShader*      CRenderer::m_DefaultPixelShader = NULL;
ID3D11InputLayout*      CRenderer::m_DefaultVertexLayout = NULL;
...
float CRenderer::m_GloabalAlpha = 1;
...
void CRenderer::Init()
{
	...
	// 頂点シェーダ生成
	{
		...
		m_D3DDevice->CreateVertexShader(buffer, fsize, NULL, &m_DefaultVertexShader);
	...
	// ピクセルシェーダ生成
	{
		...
		m_D3DDevice->CreatePixelShader(buffer, fsize, NULL, &m_DefaultPixelShader);
	...
	// 入力レイアウト設定
	m_ImmediateContext->IASetInputLayout( m_DefaultVertexLayout );

	// シェーダ設定
	m_ImmediateContext->VSSetShader( m_DefaultVertexShader, NULL, 0 );
	m_ImmediateContext->PSSetShader( m_DefaultPixelShader, NULL, 0 );
...
void CRenderer::Uninit()
{
	...
	m_DefaultVertexLayout->Release();
	m_DefaultVertexShader->Release();
	m_DefaultPixelShader->Release();
...
{
	// バックバッファクリア
	float ClearColor[4] = { 0.0f, 0, 0.0f, 1.0f };
...
void CRenderer::SetWorldViewProjection2D(D3DXMATRIX * WorldMatrix)
{
...
{
	if (Material.Ambient.a>m_GloabalAlpha)
	{
		Material.Ambient.a = m_GloabalAlpha;
	}
	if (Material.Diffuse.a > m_GloabalAlpha)
	{
		Material.Diffuse.a = m_GloabalAlpha;
	}
	if (Material.Emission.a> m_GloabalAlpha)
	{
		Material.Emission.a = m_GloabalAlpha;
	}
	if (Material.Specular.a > m_GloabalAlpha)
	{
		Material.Specular.a = m_GloabalAlpha;
	}
...
void CRenderer::SetGloabalAlpha(float set)
{
	if (set>1)
	{
		return;
...
void CRenderer::SetShader(ID3D11VertexShader *vertexShader, ID3D11PixelShader *pixelShader, ID3D11InputLayout* vertexLayout)
{
	m_ImmediateContext->IASetInputLayout(vertexLayout);
	m_ImmediateContext->VSSetShader(vertexShader, NULL, 0);
	m_ImmediateContext->PSSetShader(pixelShader, NULL, 0);
}


float CRenderer::GetGloabalAlpha()
{
	return m_GloabalAlpha;
}