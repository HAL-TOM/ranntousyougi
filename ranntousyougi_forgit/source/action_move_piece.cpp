...
void CActionMovePiece::Setting(CPlayerBase * player, CPlayerBase* enemy, CSquare * pieceSquare, CSquare * targetSquare)
{
	m_player = player;
	m_enemy = enemy;
...
int CActionMovePiece::CreateValue()
{
	int ret = 0;
		...
		for (int y = 0; y < SQUARE_TABLE_MAX_Y + 1; y++)
		{
			for (int x = 0; x < SQUARE_TABLE_MAX_X + 1; x++)
			{
				...
				if (originalTable->GetSquare(x, y)->GetPiece() != nullptr)
				{
					...
					piece->SetSquareUnCmoponent(square[x][y]);
